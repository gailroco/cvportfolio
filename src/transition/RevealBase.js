...
class RevealBase extends React.Component {
  ...
  constructor(props,
    ...
    this.state = {
      ...
      style:
      {
        opacity: ( ( (!this.isOn) || (props.ssrReveal) ) &&
                   (props.outEffect)
                 ) ? 0 : void 0,
      },
    };
  ...
  animationEnd(func,
    ...
    this.animationEndTimeout = window.setTimeout(
                                handler,
                                delay +
                                  (duration + (cascade ? duration : 0) * count)
                               );

  }

  getDimensionValue() {
    let tmp1 = this.el.offsetHeight,
        tmp2 = parseInt(
                window.getComputedStyle(this.el, null).getPropertyValue(
                  'margin-top'), 10),
        tmp3 = parseInt(
                window.getComputedStyle(this.el, null).getPropertyValue(
                  'margin-bottom'), 10)
  ...
  collapse(state,
    ...
    if (props.collapseOnly) {
      ...
      delay =
        inOut.delay + ( (this.isOn) ? 0 : total - delta1 - delta2 );

      state.style.animationDuration =
        `${total - delta1 + ( (this.isOn) ? delta2 : -delta2 )}ms`;

      state.style.animationDelay =
        `${inOut.delay + ( (this.isOn) ? delta1 - delta2 : 0 )}ms`;
    }
  ...
  animate(props) {
    ...
    this.setState((props.collapse)
                    ? this.collapse(state, props, inOut) : state);
  ...
  onReveal(props) {
    if ( (props.onReveal) && (this.isOn) ) {
      ...
      props.wait ? this.onRevealTimeout = window.setTimeout(props.onReveal,
                                                            props.wait)
                 : props.onReveal();
    }
  }
  ...
  cascade(children) {
    ...
    let { duration, reverse } =
      this.props[( (this.isOn) || (!this.props.outEffect) )
                 ? 'inEffect'
                 : 'outEffect'];
        ...
        ? React.cloneElement(child,
          {
            style:
            {
              ...child.props.style,
              ...this.state.style,
              animationDuration: Math.round(cascade(reverse ? i-- : i++,
                                                    0,
                                                    count,
                                                    duration,
                                                    total)) + 'ms',
  ...
  static getInitialCollapseStyle
  (
    props
  ) {
    return {
  ...
  (props
    ...
    if ( (props.collapse) && (!this.props.collapse) ) {
      this.setState({
                      style: {},
                      collapse: RevealBase.getInitialCollapseStyle(props)
                    });
      this.isShown = false;
    }

    if ( (props.when !== this.props.when) ||
         (props.spy !== this.props.spy)
       ) {
      this.reveal(props);
    }
  ...
  render() {
    ...
    if (!this.state.hasAppeared) {
      ...
      mount =
        (!this.props.unmountOnExit) || (this.isOn) || (!this.state.hasExited);
    }
    ...
    let newClass = 
      (this.props.disabled)
        ? className
        : `${ this.props.outEffect ?
                namespace : '' }${ this.state.className ?
                  ' ' +
    ...
    if (this.props.collapse !== undefined) {
      return this.props.collapseEl
        ...
        : <div style={ (this.props.disabled)
                          ? undefined
                          : this.state.collapse } children={el}
          />;
    }
  ...
  (
    ...
    const h = this.el.offsetHeight,
          delta = window.scrollY - RevealBase.getTop(this.el),
          tail = Math.min(h,
                          window.innerHeight) *
                ( globalHide ? props.fraction : 0 );
  ...
  resize(props) {
    ...
    if ( this.inViewport(props) ) {
      ...
      this.setState({
                      ...
                      style:
                      {
                        opacity: ( (this.isOn) || (!props.outEffect) )
                                  ? 1 : 0
                      }
                    });
      this.onReveal(props);
  ...
  listen() {
    if ( (!observerMode) && (!this.isListener) ) {
      ...
      window.document.addEventListener('collapseend',
                                       this.revealHandler,
                                       { passive: true });
  ...
  unlisten() {
    if ( (!observerMode) && (this.isListener) ) {
      ...
      window.document.removeEventListener('collapseend',
                                          this.revealHandler,
                                          { passive: true });
